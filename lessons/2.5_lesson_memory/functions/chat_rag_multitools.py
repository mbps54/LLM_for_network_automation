...
def cmdb_tool(name: str) -> str:
    ...
    Если IP не найден — используй lookup_docs().
    """
    ip = cmdb(name)
    if not ip:
...
def build_prompt_with_system_prompt(user_input: str) -> str:
    """
    Формирует текст запроса с системным сообщением и текущим вводом пользователя.
    """
    system_prompt = (
    ...
    return f"[Системное сообщение]: {system_prompt}\n[Пользователь]: {user_input}"
